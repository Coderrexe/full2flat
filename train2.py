...
from models.model import Generator, GlobalGenerator2, InceptionV3, networks
...
netD_B = networks.define_D(D_input_nc_B, opt.num_discrim_filters, opt.netD, opt.n_layers_D, opt.norm, use_sigmoid=False)
netD_A = networks.define_D(D_input_nc_A, opt.num_discrim_filters, opt.netD, opt.n_layers_D, opt.norm, use_sigmoid=False)
...
# Load pretrained Inception - only needed for depth prediction
...
train_dataloader = DataLoader(train_ds, batch_size=opt.batchSize, shuffle=True, num_workers=opt.n_cpu, drop_last=True)
...
for epoch in range(opt.epoch, opt.n_epochs):
        ...
        if (i + 1) % opt.log_int == 0:
            ...
            if opt.use_geom:
                errors["loss_cycle_Geom"] = torch.mean(loss_cycle_Geom) if not isinstance(loss_cycle_Geom,
                                                                                          (int, float)) else loss_cycle_Geom
    ...
    # save_image(rec_B.data, f"results/exp2/generated_imgs/epoch_{epoch+1}_rec_B.png", normalize=True)
    # save_image(rec_A.data, f"results/exp2/generated_imgs/epoch_{epoch+1}_rec_A.png", normalize=True)

    end_time = time.time()
    elapsed_time = round(end_time - start_time, 1)

"""
python train.py --name exp4 \
--dataroot examples/train/full_color \
--root2 examples/train/flat_color \
--no_flip --cuda --n_epochs 200 --decay_epoch 100 --batchSize 6 --wandb --save_epoch_freq 1 --use_geom 0 --midas 0
