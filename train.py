...
if __name__ == "__main__":
    ...
    # Loading data
    parser.add_argument("--dataroot", type=str, default="datasets/vangogh2photo/",
    ...
    netD_B = networks.define_D(D_input_nc_B, opt.num_discrim_filters, opt.netD, opt.n_layers_D, opt.norm,
                               use_sigmoid=False)
    netD_A = networks.define_D(D_input_nc_A, opt.num_discrim_filters, opt.netD, opt.n_layers_D, opt.norm,
                               use_sigmoid=False)
    ...
    # Losses
    criterionGAN = networks.GANLoss(use_lsgan=True, target_real_label=1.0, target_fake_label=0.0, reduceme=True).to(
        device)
    ...
    train_ds = UnpairedDepthDataset(opt.dataroot, opt.root2, opt, transforms_r=transforms_r,
                                    mode=opt.mode, midas=opt.midas > 0, depthroot=opt.depthroot, sketchroot="examples/train/line_drawings")

    train_dataloader = DataLoader(train_ds, batch_size=opt.batchSize, shuffle=True, num_workers=opt.n_cpu,
                                  drop_last=True)
    ...
    for epoch in range(opt.epoch, opt.n_epochs):
        ...
        save_image(fake_B.data, f"generated_images/exp8/epoch_{epoch + 1}_fake_B.png", normalize=True)
        save_image(fake_A.data, f"generated_images/exp8/epoch_{epoch + 1}_fake_A.png", normalize=True)
        save_image(rec_B.data, f"generated_images/exp8/epoch_{epoch + 1}_rec_B.png", normalize=True)
        save_image(rec_A.data, f"generated_images/exp8/epoch_{epoch + 1}_rec_A.png", normalize=True)

        end_time = time.time()
        elapsed_time = round(end_time - start_time, 1)

    """
    python train.py --name exp8 --dataroot examples/train/full_color --root2 examples/train/flat_color --no_flip --cuda --n_epochs 150 --decay_epoch 75 --batchSize 6 --wandb 0 --save_epoch_freq 1 --use_geom 0 --midas 0 --lr 0.0002 --use_clip 0 --cond_cycle 10.0 --use_sketch 1
